<x-public-layout :hideFooter="true">
    ...
    <div class="bg-gray-50 dark:bg-background-dark min-h-screen -mt-20 pt-32 pb-24 font-inter">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex text-xs md:text-sm text-gray-400 mb-6 space-x-2 no-print">
                ...
                <span>/</span>
                <span class="text-gray-800 dark:text-gray-200 font-medium">{{ __('Tickets.Confirmation.Title') }}</span>
            </nav>
                ...
                <div class="p-6 md:p-8">
                    ...
                    <div class="mb-5 pb-5 border-b border-slate-100 dark:border-slate-700">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">{{ __('Tickets.Confirmation.OrderNo') }}</p>
                            ...
                            <div>
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">{{ __('Tickets.Confirmation.TicketNo') }}</p>
                    ...
                    <div class="grid grid-cols-2 gap-x-4 gap-y-4 mb-5 pb-5 border-b border-slate-100 dark:border-slate-700">
                        <div>
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">{{ __('Tickets.Confirmation.TicketItem') }}</p>
                            <p class="font-semibold text-sm text-slate-900 dark:text-white">{{ $order->ticket->name }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">{{ __('Tickets.Confirmation.Destination') }}</p>
                            <p class="font-semibold text-sm text-slate-900 dark:text-white">{{ $order->ticket->place->name }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">{{ __('Tickets.Confirmation.VisitDate') }}</p>
                            <p class="font-semibold text-sm text-slate-900 dark:text-white">{{ $order->visit_date->translatedFormat('d F Y') }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">{{ __('Tickets.Confirmation.Amount') }}</p>
                            <p class="font-semibold text-sm text-slate-900 dark:text-white">{{ $order->quantity }} {{ __('Tickets.Unit') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mb-5 pb-5 border-b border-slate-100 dark:border-slate-700">
                        <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                            ...
                            {{ __('Tickets.Confirmation.CustomerInfo') }}
                        </p>
                        <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between mb-5 pb-5 border-b border-slate-100 dark:border-slate-700">
                        <span class="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">{{ __('Tickets.Confirmation.Total') }}</span>
                    ...
                    <div class="text-center mb-5 pb-5 border-b border-slate-100 dark:border-slate-700">
                        <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-4 flex items-center justify-center gap-1.5">
                            ...
                            {{ __('Tickets.Confirmation.QRCode') }}
                        </p>
                        ...
                        <div class="relative inline-block">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center bg-white/60 dark:bg-slate-800/60 rounded-lg">
                                <span class="bg-blue-600/90 text-white text-[10px] font-black uppercase tracking-[0.2em] px-4 py-1.5 rounded-lg rotate-[-15deg] shadow-lg">
                                    {{ __('Tickets.Confirmation.UsedStamp') }}
                                </span>
                            </div>
                            @endif
                        </div>
                        ...
                        @if($order->status === 'paid')
                        <p class="text-xs text-slate-400 dark:text-slate-500 mb-3">{{ __('Tickets.Confirmation.ShowQR') }}</p>
                        <button onclick="downloadQR()" class="text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-primary transition-colors inline-flex items-center gap-1.5">
                            ...
                            {{ __('Tickets.Confirmation.DownloadQR') }}
                        </button>
                        @endif
                    </div>
                    @endif
                    ...
                    <div class="rounded-xl p-4 mb-5 {{ $config['bg'] }} {{ $config['border'] }} border text-left">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="text-sm text-slate-600 dark:text-slate-300">
                                <p class="font-bold mb-1.5 text-slate-800 dark:text-white text-xs uppercase tracking-wider">{{ __('Tickets.Confirmation.NextSteps') }}</p>
                                <ul class="list-disc list-inside space-y-0.5 text-xs">
                                    <li>{!! __('Tickets.Confirmation.Step1', ['email' => '<strong>'.$order->customer_email.'</strong>']) !!}</li>
                                    <li>{{ __('Tickets.Confirmation.Step2') }}</li>
                                    <li>{{ __('Tickets.Confirmation.Step3', ['place' => $order->ticket->place->name]) }}</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl p-4 mb-5 {{ $config['bg'] }} {{ $config['border'] }} border text-left"
                        ...
                        <div class="flex items-start gap-3 mb-4">
                            ...
                            <div class="text-sm text-slate-600 dark:text-slate-300 w-full">
                                <div class="flex justify-between items-start">
                                    <p class="font-bold mb-1 pt-0.5 text-slate-800 dark:text-white text-xs uppercase tracking-wider">{{ __('Tickets.Confirmation.PayWithin') }}</p>
                        ...
                        <div class="bg-white dark:bg-slate-800/50 rounded-lg p-4 border border-yellow-200 dark:border-yellow-800/50">
                            <p class="text-xs text-slate-500 mb-1 uppercase tracking-wide">{{ __('Tickets.Confirmation.PaymentMethod') }}</p>
                            <p class="font-bold text-slate-900 dark:text-white text-base mb-3 flex items-center gap-2">
                                @if($order->payment_method_detail === 'bank_transfer')
                                    {{ __('Tickets.Confirmation.BankTransfer') }} {{ strtoupper($order->payment_channel) }}
                                @elseif($order->payment_method_detail === 'gopay')
                                    {{ __('Tickets.Confirmation.Gopay') }}
                                @elseif($order->payment_method_detail === 'qris')
                                    {{ __('Tickets.Confirmation.Qris') }}
                                @elseif($order->payment_method_detail === 'echannel')
                                    {{ __('Tickets.Confirmation.MandiriBillPayment') }}
                                @else
                            ...
                            @if(isset($order->payment_info['va_number']))
                                <div class="mb-3">
                                    <p class="text-xs text-slate-500 mb-1">{{ __('Tickets.Confirmation.VA') }}</p>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button onclick="navigator.clipboard.writeText('{{ $order->payment_info['va_number'] }}'); alert('{{ __('Tickets.Confirmation.Alert.Copied') }}');" class="text-primary hover:text-primary/80">
                            ...
                            @elseif(isset($order->payment_info['bill_key']))
                                <div class="mb-3">
                                    <p class="text-xs text-slate-500 mb-1">{{ __('Tickets.Confirmation.CompanyCode') }}</p>
                                    <p class="font-mono text-lg font-bold text-slate-800 dark:text-slate-200 mb-2">{{ $order->payment_info['biller_code'] }}</p>

                                    <p class="text-xs text-slate-500 mb-1">{{ __('Tickets.Confirmation.PaymentCode') }}</p>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button onclick="navigator.clipboard.writeText('{{ $order->payment_info['bill_key'] }}'); alert('{{ __('Tickets.Confirmation.Alert.Copied') }}');" class="text-primary hover:text-primary/80">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 01-1.125-1.125V7.875c0-.621.504-1.125 1.125-1.125H6.75a9.06 9.06 0 011.5.124m7.5 10.376h3.375c.621 0 1.125-.504 1.125-1.125V11.25c0-4.46-3.243-8.161-7.5-8.876a9.06 9.06 0 00-1.5-.124H9.375c-.621 0-1.125.504-1.125 1.125v3.5m7.5 10.375H9.375a1.125 1.125 0 01-1.125-1.125v-9.25m12 6.625v-1.875a3.375 3.375 0 00-3.375-3.375h-1.5a1.125 1.125 0 01-1.125-1.125v-1.5a3.375 3.375 0 00-3.375-3.375H9.75" /></svg>
                                        </button>
                                    </div>
                                </div>
                            @elseif(isset($order->payment_info['qr_url']))
                                <div class="text-center mb-3">
                                    ...
                                    <p class="text-xs text-slate-500 mt-2">{{ __('Tickets.Confirmation.ScanQR') }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl p-4 mb-5 {{ $config['bg'] }} {{ $config['border'] }} border text-left">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="text-sm text-slate-600 dark:text-slate-300">
                                <p class="font-bold mb-1 text-slate-800 dark:text-white text-xs uppercase tracking-wider">{{ __('Tickets.Confirmation.Cancelled') }}</p>
                                <p class="text-xs">{{ __('Tickets.Confirmation.CancelledMsg') }}</p>
                            </div>
                        </div>
                    </div>
                    @elseif($order->status === 'used')
                    <div class="rounded-xl p-4 mb-5 {{ $config['bg'] }} {{ $config['border'] }} border text-left">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="text-sm text-slate-600 dark:text-slate-300">
                                <p class="font-bold mb-1 text-slate-800 dark:text-white text-xs uppercase tracking-wider">{{ __('Tickets.Confirmation.UsageDetail') }}</p>
                                <ul class="list-disc list-inside space-y-0.5 text-xs">
                                    <li>{{ __('Tickets.Confirmation.Destination') }}: <strong>{{ $order->ticket->place->name }}</strong></li>
                                    @if($order->check_in_time)
                                    <li>{{ __('Tickets.Confirmation.CheckIn') }}: <strong>{{ $order->check_in_time->translatedFormat('d F Y, H:i') }}</strong></li>
                                    @endif
                                    <li>{{ __('Tickets.Confirmation.OneTime') }}</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3 no-print" x-data="{ showCancelConfirm: false }">
                        @if($order->status === 'pending')
                            ...
                            <a href="{{ route('payment.status', $order->order_number) }}"
                               class="w-full bg-primary hover:bg-primary/90 text-white font-bold py-3.5 rounded-2xl transition-all duration-300 shadow-lg shadow-primary/25 flex items-center justify-center gap-2 text-sm">
                                ...
                                {{ __('Tickets.Confirmation.Button.Pay') }}
                            </a>
                            @else
                            <a href="{{ route('payment.show', $order->order_number) }}"
                               class="w-full bg-primary hover:bg-primary/90 text-white font-bold py-3.5 rounded-2xl transition-all duration-300 shadow-lg shadow-primary/25 flex items-center justify-center gap-2 text-sm">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" /></svg>
                                {{ __('Tickets.Confirmation.Button.PayNow') }}
                            </a>
                            @endif
                            <div class="flex gap-3">
                                <button onclick="
                                    this.innerHTML = '<svg class=\'w-4 h-4 animate-spin\' fill=\'none\' viewBox=\'0 0 24 24\'><circle class=\'opacity-25\' cx=\'12\' cy=\'12\' r=\'10\' stroke=\'currentColor\' stroke-width=\'4\'></circle><path class=\'opacity-75\' fill=\'currentColor\' d=\'M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z\'></path></svg> {{ __('Tickets.Confirmation.Button.Checking') }}';
                                    this.disabled = true;
                                    const btn = this;
                                    fetch('{{ route('payment.check', $order->order_number) }}')
                                        ...
                                        .then(d => {
                                            if(d.status === 'paid') { window.location.reload(); }
                                                ...
                                                setTimeout(() => { btn.innerHTML = '<svg class=\'w-4 h-4\' fill=\'none\' viewBox=\'0 0 24 24\' stroke-width=\'1.5\' stroke=\'currentColor\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' d=\'M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182\' /></svg> {{ __('Tickets.Confirmation.Button.CheckStatus') }}'; btn.disabled = false; }, 3000);
                                            }
                                        })
                                        .catch(() => { btn.innerHTML = '<svg class=\'w-4 h-4\' fill=\'none\' viewBox=\'0 0 24 24\' stroke-width=\'1.5\' stroke=\'currentColor\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' d=\'M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182\' /></svg> {{ __('Tickets.Confirmation.Button.CheckStatus') }}'; btn.disabled = false; });
                                " class="flex-1 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 font-semibold py-3 rounded-2xl transition-all duration-300 flex items-center justify-center gap-2 text-sm">
                                    ...
                                    {{ __('Tickets.Confirmation.Button.CheckStatus') }}
                                </button>
                                <button @click="showCancelConfirm = true"
                                    ...
                                    {{ __('Tickets.Confirmation.Button.Cancel') }}
                                </button>
                            </div>
                            ...
                            <div x-show="showCancelConfirm" x-cloak
                                ...
                                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center" @click.away="showCancelConfirm = false">
                                    ...
                                    <h3 class="font-bold text-lg text-slate-900 dark:text-white mb-2">{{ __('Tickets.Confirmation.CancelOrderQuestion') }}</h3>
                                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">{{ __('Tickets.Confirmation.CancelOrderMessage', ['order_number' => $order->order_number]) }}</p>
                                    <div class="flex gap-3">
                                        <button @click="showCancelConfirm = false" class="flex-1 px-4 py-2.5 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 font-semibold rounded-xl transition-colors text-sm">{{ __('Tickets.Confirmation.Button.Back') }}</button>
                                        <form action="{{ route('payment.cancel', $order->order_number) }}" method="POST" class="flex-1">
                                            @csrf
                                            <button type="submit" class="w-full px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-xl transition-colors text-sm">{{ __('Tickets.Confirmation.Button.YesCancel') }}</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        ...
                        @elseif($order->status === 'paid')
                            <button onclick="downloadTicketImage()" id="downloadBtn"
                               class="w-full bg-primary hover:bg-primary/90 text-white font-bold py-3.5 rounded-2xl transition-all duration-300 shadow-lg shadow-primary/25 flex items-center justify-center gap-2 text-sm">
                                ...
                                {{ __('Tickets.Confirmation.Button.DownloadTicket') }}
                            </button>
                        ...
                        @elseif($order->status === 'cancelled')
                            <a href="{{ route('tickets.index') }}"
                               class="w-full bg-primary hover:bg-primary/90 text-white font-bold py-3.5 rounded-2xl transition-all duration-300 shadow-lg shadow-primary/25 flex items-center justify-center gap-2 text-sm">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 6v.75m0 3v.75m0 3v.75m0 3V18m-9-5.25h5.25M7.5 15h3M3.375 5.25c-.621 0-1.125.504-1.125 1.125v3.026a2.999 2.999 0 010 5.198v3.026c0 .621.504 1.125 1.125 1.125h17.25c.621 0 1.125-.504 1.125-1.125v-3.026a2.999 2.999 0 010-5.198V6.375c0-.621-.504-1.125-1.125-1.125H3.375z" /></svg>
                                {{ __('Tickets.Confirmation.Button.NewOrder') }}
                            </a>
                        @endif
                        ...
                        <a href="{{ route('tickets.my') }}"
                           class="w-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 font-semibold py-3 rounded-2xl transition-all duration-300 flex items-center justify-center gap-2 text-sm">
                            ...
                            {{ __('Tickets.Confirmation.Button.BackToMyTickets') }}
                        </a>
                    </div>
    ...
    {{-- ═══════════════════════════════════════════════════════
         ...
         Matches download.blade.php design exactly
         ═══════════════════════════════════════════════════════ --}}
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (hiddenQrElement) {
            ...
            // Fix for QRCodeJS styling
            setTimeout(() => {
                const img = hiddenQrElement.querySelector('img');
                if(img) { 
                    img.style.width = '100%'; 
    ...
    function downloadTicketImage() {
        ...
        const images = ticketCard.querySelectorAll('img');
        ...
        Promise.all(promises).then(() => {
            html2canvas(ticketCard, { 
                scale: 3, 
                useCORS: true, 
                backgroundColor: null, // Allow transparency/gradients
                logging: false
            }).then(canvas => {
                const link = document.createElement('a');
