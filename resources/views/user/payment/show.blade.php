<x-public-layout :hideFooter="true">
    {{-- Scrollbar-hide utility for mobile horizontal scroll --}}
    <style>
    ...
    <div class="bg-gray-50 dark:bg-background-dark min-h-screen -mt-20 pt-24 sm:pt-32 pb-32 sm:pb-24">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 p-4 sm:p-6 md:p-8">
                ...
                <div class="text-center mb-5 sm:mb-8">
                    <div class="w-12 h-12 sm:w-16 sm:h-16 bg-primary/10 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4">
                        <i class="fa-solid fa-credit-card text-primary text-xl sm:text-2xl"></i>
                    </div>
                    <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-1 sm:mb-2">Pilih Metode Pembayaran</h1>
                    <p class="text-slate-500 dark:text-slate-400 text-sm hidden sm:block">Pilih metode pembayaran yang paling nyaman untuk Anda</p>
                </div>

                {{-- Order Summary — compact on mobile, total-focused --}}
                <div class="bg-slate-50 dark:bg-slate-700/30 rounded-xl sm:rounded-2xl p-4 sm:p-5 mb-5 sm:mb-8">
                    <div class="flex justify-between items-center sm:pb-4 sm:border-b sm:border-slate-200 sm:dark:border-slate-600">
                        <div>
                            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-0.5">Pesanan</p>
                            <p class="font-bold text-slate-900 dark:text-white font-mono text-xs sm:text-sm">{{ $order->order_number }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-0.5">Total</p>
                            <p class="text-xl sm:text-lg font-bold text-primary">Rp {{ number_format($order->total_price, 0, ',', '.') }}</p>
                        </div>
                    </div>
                    {{-- Ticket detail — hidden on mobile for compactness --}}
                    ...
                    {{-- Mobile-only compact ticket info --}}
                    <div class="flex sm:hidden mt-3 pt-3 border-t border-slate-200 dark:border-slate-600 text-xs text-slate-500 dark:text-slate-400">
                        ...
                        <span>{{ $order->quantity }}x {{ $order->ticket->name }} · {{ $order->visit_date->translatedFormat('d M Y') }}</span>
                    </div>
                </div>
                ...
                <form action="{{ route('payment.process', $order->order_number) }}" method="POST" id="payment-form" x-data="paymentSelector()" @submit="isSubmitting = true">
                        ...
                        <div class="flex sm:grid sm:grid-cols-3 gap-3 sm:gap-4 overflow-x-auto sm:overflow-visible snap-x snap-mandatory pb-2 sm:pb-0 -mx-1 px-1 scrollbar-hide">
                            ...
                            <button type="button" @click="selectMethod('qris')"
                                ...
                                <div class="text-center w-full z-10">
                                    <p class="font-medium text-sm text-slate-900 dark:text-white mb-0.5 sm:mb-1 transition-colors group-hover:text-gray-800 dark:group-hover:text-gray-200">QRIS</p>
                                    <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wide">Scan e-wallet</p>
                                </div>

                                {{-- Selection Indicator --}}
                                <div class="absolute top-3 right-3 sm:top-4 sm:right-4 transition-all duration-200" 
                            ...
                            <button type="button" @click="selectMethod('gopay')"
                                ...
                                <div class="text-center w-full z-10">
                                    <p class="font-medium text-sm text-slate-900 dark:text-white mb-0.5 sm:mb-1 transition-colors group-hover:text-[#00AED6]" :class="paymentType === 'gopay' ? 'text-[#00AED6]' : ''">GoPay</p>
                            ...
                            <button type="button" @click="selectMethod('shopeepay')"
                                ...
                                <div class="text-center w-full z-10">
                                    <p class="font-medium text-sm text-slate-900 dark:text-white mb-0.5 sm:mb-1 transition-colors group-hover:text-[#EE4D2D]" :class="paymentType === 'shopeepay' ? 'text-[#EE4D2D]' : ''">ShopeePay</p>
                                    <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wide">Instant Checkout</p>
                                </div>
                    ...
                    <div class="mb-5 sm:mb-8 relative">
                        <div class="mb-3">
                            <h3 class="text-xs sm:text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-2">
                                ...
                                <span class="hidden sm:inline-flex bg-yellow-100 text-yellow-700 text-[10px] py-0.5 px-2 rounded-full font-bold border border-yellow-200 items-center">
                                    <i class="fa-solid fa-clock mr-1"></i> Segera Hadir
                                </span>
                            </h3>
                            <span class="sm:hidden inline-flex mt-1.5 ml-5 bg-yellow-100 text-yellow-700 text-[9px] py-0.5 px-2 rounded-full font-bold border border-yellow-200 items-center">
                                <i class="fa-solid fa-clock mr-1"></i> Segera Hadir
                            </span>
                        </div>
                        
                        {{-- Disabled bank options --}}
                        <div class="flex sm:grid sm:grid-cols-4 gap-3 sm:gap-4 overflow-x-auto sm:overflow-visible pb-2 sm:pb-0 -mx-1 px-1 scrollbar-hide opacity-60 grayscale select-none pointer-events-none filter">
                            ...
                            <button type="button" disabled
                                class="relative flex flex-col items-center justify-center gap-3 p-3 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 cursor-not-allowed group h-20 sm:h-24 min-w-[100px] sm:min-w-0 flex-shrink-0 sm:flex-shrink">
                                <img src="{{ asset('images/payment/bca.png') }}" alt="BCA" class="h-6 sm:h-8 w-auto object-contain">
                            </button>

                            {{-- BNI --}}
                            <button type="button" disabled
                                class="relative flex flex-col items-center justify-center gap-3 p-3 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 cursor-not-allowed group h-20 sm:h-24 min-w-[100px] sm:min-w-0 flex-shrink-0 sm:flex-shrink">
                                <img src="{{ asset('images/payment/bni.png') }}" alt="BNI" class="h-6 sm:h-8 w-auto object-contain">
                            </button>

                            {{-- BRI --}}
                            <button type="button" disabled
                                class="relative flex flex-col items-center justify-center gap-3 p-3 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 cursor-not-allowed group h-20 sm:h-24 min-w-[100px] sm:min-w-0 flex-shrink-0 sm:flex-shrink">
                                <img src="{{ asset('images/payment/bri.png') }}" alt="BRI" class="h-6 sm:h-8 w-auto object-contain">
                            </button>

                            {{-- Mandiri --}}
                            <button type="button" disabled
                                class="relative flex flex-col items-center justify-center gap-3 p-3 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 cursor-not-allowed group h-20 sm:h-24 min-w-[100px] sm:min-w-0 flex-shrink-0 sm:flex-shrink">
                                <img src="{{ asset('images/payment/mandiri.png') }}" alt="Mandiri" class="h-6 sm:h-8 w-auto object-contain">
                            </button>
                        </div>
                    ...
                    <div class="fixed sm:hidden bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-slate-800/95 backdrop-blur-lg p-4 border-t border-slate-100 dark:border-slate-700 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
                        <button type="submit" id="pay-button-mobile"
                            :disabled="!paymentType || isSubmitting"
            ...
            {{-- QRIS Info --}}
            <div class="mt-3 sm:mt-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/30 rounded-xl sm:rounded-2xl p-3 sm:p-4 flex items-start gap-3">
